...
export default function PedidosDelivery() {
  ...
  const handleConfirmarCobroDelivery = async (e) => {
    ...
    const numPed = pedido.numeroPedido || pedido.numero || targetId?.slice?.(-4) || '---';
    const medioPagoBackend = (medioPago === 'tarjeta' || medioPago === 'debito_credito')
      ? 'debito_credito'
      : (medioPago === 'transferencia' ? 'transferencia' : 'efectivo');

    let res;
    // 1. Petición principal al backend para cambiar estado y cerrar
    try {
      ...
      res = await apiClient.post(`/pedidos-delivery/${targetId}/cerrar`, {
    ...
    try {
      ...
      setPedidoDetalleId(null);
      await cargarDatosDelivery();
    } catch (secErr) {
      console.error('Cobro de Delivery registrado en backend, pero falló la actualización de la vista:', secErr);
